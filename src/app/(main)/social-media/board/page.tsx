...
export default function SocialMediaBoardPage() {
  ...
  const filteredPosts = useMemo(() => {
    ...
    if (profile?.role === 'Super Admin' && selectedBrands.length > 0) {
      filtered = filtered.filter(post => post.brandId && selectedBrands.includes(post.brandId));
    } else if (profile?.role === 'Manager' && selectedUsers.length > 0) {
      filtered = filtered.filter(post => post.assigneeIds.some(assigneeId => selectedUsers.includes(assigneeId)));
    }
    return filtered;
  }, [posts, selectedUsers, selectedBrands, profile]);
  ...
  const isLoading = isProfileLoading || arePostsLoading || areUsersLoading || areBrandsLoading;
  ...
  return (
    <div className="flex h-full flex-col">
        ...
        <GenericKanbanBoard 
            ...
            users={allUsers || []}
            isLoading={isLoading} 
            itemType="socialMediaPosts" 
            statusCollection="socialMediaStatuses" 
        />
    </div>
  );
}
