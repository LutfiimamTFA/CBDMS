...
export function ShareViewDialog({ children, navItems }: ShareViewDialogProps) {
  ...
  const [accessLevel, setAccessLevel] =
    useState<SharedLink['accessLevel']>('view');
  const [selectedNavIds, setSelectedNavIds] = useState<string[]>([]);

  const isEmployeeRole =
    profile?.role === 'Employee' || profile?.role === 'PIC';
  ...
  const shareableNavItems = useMemo(() => {
    ...
    return navItems.filter(
      (item) => item.roles.includes(profile.role) && isShareable(item)
    );
  }, [navItems, profile]);
  ...
  const userTasksQuery = useMemo(() => {
    ...
    if (profile.role === 'Manager' && profile.brandIds?.length) {
  ...
  const webArticlesQuery = useMemo(() => {
    ...
    let q = query(
      collection(firestore, 'webArticles'),
      where('companyId', '==', profile.companyId)
    );
  ...
  const { data: allVisibleTasks, isLoading: tasksLoading } =
    useCollection<Task>(userTasksQuery);

  const { data: allVisibleSocialPosts, isLoading: socialPostsLoading } =
    useCollection<SocialMediaPost>(socialPostsQuery);

  const { data: allVisibleWebArticles, isLoading: webArticlesLoading } =
    useCollection<WebArticle>(webArticlesQuery);

  useEffect(() => {
    if (!isOpen) return;

    const dashboardItem = shareableNavItems.find(
      (item) => item.id === 'nav_dashboard' || item.path === '/dashboard'
    );

    if (dashboardItem) {
      setSelectedNavIds([dashboardItem.id]);
    } else {
      setSelectedNavIds(shareableNavItems.slice(0, 1).map((item) => item.id));
    }
  ...
  const handleCreateLink = async () => {
    ...
    try {
      ...
      const socialStatusesQuery = query(
        collection(firestore, 'socialMediaStatuses'),
        orderBy('order')
      );
      const webStatusesQuery = query(
        collection(firestore, 'webStatuses'),
        orderBy('order')
      );
      ...
      if (statusesSnap.empty) {
        throw new Error(
          'Cannot create share link: A valid workflow was not found for this company.'
        );
      }
      ...
      const snapshot = {
        ...
        statuses: statusesSnap.docs.map(
          (docSnap) =>
            ({ id: docSnap.id, ...docSnap.data() }) as WorkflowStatus
        ),
        socialMediaStatuses: socialStatusesSnap.docs.map(
          (docSnap) =>
            ({ id: docSnap.id, ...docSnap.data() }) as WorkflowStatus
        ),
        webStatuses: webStatusesSnap.docs.map(
          (docSnap) =>
            ({ id: docSnap.id, ...docSnap.data() }) as WorkflowStatus
        ),
        ...
        brands: brandsSnap.docs.map(
          (docSnap) => ({ id: docSnap.id, ...docSnap.data() }) as Brand
        ),
      };

      const linkData: Omit<SharedLink, 'id' | 'createdAt'> = {
        name: linkName || 'Shared Work View',
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Share Work View</DialogTitle>
          <DialogDescription>
            Create a public link to share a snapshot of your current work view
        ...
        {!generatedLink ? (
            ...
            <div className="space-y-6 py-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder="e.g., Q3 Client Preview"
                />
              </div>
              ...
              >
                <AccordionItem value="pages" className="rounded-lg border">
                  ...
                  <AccordionContent className="space-y-4 p-4 pt-0">
                    <div className="space-y-2">
                      ...
                      {shareableNavItems.length === 0 && (
                        <p className="text-sm text-muted-foreground">
                          No shareable pages are available for your role.
                        </p>
                      )}
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <RadioGroup
                        value={accessLevel}
                        onValueChange={(value) =>
                          setAccessLevel(value as SharedLink['accessLevel'])
                        }
                      >
                        <div className="flex items-start space-x-2 rounded-md border p-3 hover:bg-accent hover:text-accent-foreground has-[:checked]:bg-accent has-[:checked]:text-accent-foreground">
                          ...
                          <Label
                            htmlFor="perm-view"
                            className="flex cursor-pointer flex-col gap-1 leading-normal"
                          >
                            ...
                            <span className="text-xs font-normal text-muted-foreground">
                              Can view pages and task details. Cannot make any
                              changes.
                            </span>
                          </Label>
                        </div>
                        ...
                        <div className="flex items-start space-x-2 rounded-md border p-3 hover:bg-accent hover:text-accent-foreground has-[:checked]:bg-accent has-[:checked]:text-accent-foreground">
                          ...
                          >
                            ...
                            <span className="text-xs font-normal text-muted-foreground">
                              Can view pages, change task statuses, and request
                              revisions.{' '}
                              {isEmployeeRole && '(Revisi & Done disabled)'}
                            </span>
                          </Label>
                        </div>
                          ...
                          >
                            <span
                              className={cn(
                                ...
                                !isEmployeeRole && 'text-foreground'
                              )}
                            >
                            ...
                            <span className="text-xs font-normal">
                              Can change status, due date, and priority. Only
                              for Managers/Admins.
                            </span>
                          </Label>
                        </div>
          ...
          <div className="space-y-4 py-4">
            <Alert>
              ...
              <AlertDescription>
                Anyone with this link can now view the selected pages with the
          ...
          {generatedLink ? (
            ...
            <>
              ...
              <Button
                ...
                disabled={isLoading || anyLoading || selectedNavIds.length === 0}
              >
