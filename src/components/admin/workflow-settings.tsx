...
interface WorkflowSettingsProps {
    collectionName: 'statuses' | 'socialMediaStatuses' | 'webStatuses';
    workstreamTitle: 'Task' | 'Social Media' | 'Web';
}
...
const defaultStatuses: Omit<WorkflowStatus, 'id' | 'companyId'>[] = [
    ...
    { name: 'Revisi', order: 3, color: '#f97316' },
    { name: 'Done', order: 4, color: '#22c55e' },
];

export function WorkflowSettings({ collectionName, workstreamTitle }: WorkflowSettingsProps) {
  ...
  const seedInitialStatuses = async () => {
    if (!statusesCollectionRef || !firestore || !profile) return;
    ...
    if (snapshot.empty) {
        ...
        toast({
            ...
            description: `Default statuses for ${workstreamTitle} have been created.`
        })
    } else {
        toast({
            title: 'Already Initialized',
            description: 'Default statuses have already been created for this workflow.',
            variant: 'default'
        })
    }
  };
  ...
  if (isLoading || isDbStatusesLoading) {
    ...
    <>
      <div className="flex items-center justify-between mb-4">
        ...
        <div className='flex gap-2'>
            <Button variant="outline" onClick={seedInitialStatuses}>Seed Defaults</Button>
