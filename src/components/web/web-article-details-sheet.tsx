...
export function WebArticleDetailsSheet({ 
  ...
  const canUploadDeliverables = useMemo(() => {
    if (!currentUser) return false;
    if (isEmployeeOrPIC && articleState.statusInternal === 'Preview') return false;
    return isAssignee || isManagerOrAdmin;
  }, [isAssignee, isManagerOrAdmin, isEmployeeOrPIC, articleState.statusInternal, currentUser]);
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>, fileType: 'attachment' | 'deliverable') => {
      ...
      const validatedFiles = files.filter(file => {
          ...
          if (!isAllowedType) {
            toast({
              ...
              description: `Tipe file "${file.name}" tidak dapat diunggah. Hanya gambar dan dokumen yang diizinkan.`,
              duration: 10000,
            });
            return false;
          }
      ...
      try {
          ...
          await updateDoc(doc(firestore, 'webArticles', articleState.id), { [fieldToUpdate]: [...(articleState[fieldToUpdate] || []), ...newFiles] });
                        ...
                        <div className="p-6 space-y-6">
                           ...
                           <RichTextEditor value={articleState.content || ''} onChange={() => {}} placeholder="Write your article content here..." readOnly={!canEditContent} />
                             <Tabs defaultValue="comments" className="w-full">
                                ...
                                <TabsContent value="attachments" className="mt-4 space-y-6 rounded-lg border p-4">
                                   <div>
